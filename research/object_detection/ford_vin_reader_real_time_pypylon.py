...
WORKS_NAME = "ford_vin_reader"

MODEL_NAME = 'ford_vin_reader/' + WORKS_NAME + "_1"
PATH_TO_CKPT = MODEL_NAME + '/frozen_inference_graph.pb'
PATH_TO_LABELS = os.path.join('data', WORKS_NAME + '/label.txt')

NUM_CLASSES = 18

max_boxes_to_draw = 17
min_confidence = 0.3
                ...
                if grabResult.GrabSucceeded():
                    ...
                    for i in range(min(max_boxes_to_draw, int(num_detections))):
                        if scores[0][i] >= min_confidence:
                            ...
                            vin_array.append([object_name, x_mid])
                    ...
                    sorted_vin_array = sorted(vin_array, key=lambda x: x[1])
                    [c.pop(1) for c in sorted_vin_array]

                    result_vin = ''.join(str(e[0]) for e in sorted_vin_array)

                    # print(result_vin)

                    vin_text = 'VIN: {}'.format(result_vin)
                    label_size, base_line = cv2.getTextSize(vin_text, cv2.FONT_HERSHEY_SIMPLEX, font_scale,
                                                            thickness)
                    label_ymin = max(120, label_size[1] + 10)
                    ...
                    cv2.putText(out, vin_text, (text_x_align, label_ymin - 7), cv2.FONT_HERSHEY_SIMPLEX,
                                font_scale,
                                color_move_on,
                                thickness,
                                cv2.LINE_AA)
